...
$variant = 'task0023-ec15-{0}-{1}' -f (Get-Date -Format 'yyyyMMddHHmmss'),
...
function Invoke-EC15Pass {
    ...
    $artifact = Join-Path $runs "TASK-0023-EC15-exact-content-observations-webview2-pass$Pass.json"
    Assert-NotProtectedRunArtifact -Path $artifact
    if (Test-Path -LiteralPath $artifact) {
        throw "preuve TASK-0023 deja presente; aucune suppression automatique: $artifact"
    }
